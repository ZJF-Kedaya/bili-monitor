...
async function mysqlEscapeId(name) {
  return '`' + String(name).replace(/`/g, '``') + '`';
}
async function createMysqlConn(settings) {
  var host = settings.mysqlHost || '';
  var port = Number(settings.mysqlPort) || 3306;
  var user = settings.mysqlUser || '';
  var password = settings.mysqlPass || '';
  var database = settings.mysqlDatabase || '';
  if (!host || !user || !database) throw new Error('MySQL配置不完整');
...
async function ensureMysqlTable(conn, table) {
  await conn.query('CREATE TABLE IF NOT EXISTS ' + table + ' (id BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY, t BIGINT NOT NULL, level VARCHAR(20) NOT NULL, msg TEXT NOT NULL, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4');
}
async function getLogsFromMysql(settings) {
  var table = mysqlEscapeId(settings.mysqlTable || 'bili_monitor_logs');
  var conn;
  try {
    conn = await createMysqlConn(settings);
    await ensureMysqlTable(conn, table);
    var rows = await conn.query('SELECT t, level, msg FROM ' + table + ' ORDER BY id DESC LIMIT 300');
    return (rows && rows[0] || []).map(function(r){ return { t: Number(r.t), level: r.level || 'info', msg: String(r.msg || '') }; });
  } finally {
    if (conn) { try { await conn.end(); } catch (e) {} }
  }
}
async function saveLogsToMysql(settings, logs) {
  var table = mysqlEscapeId(settings.mysqlTable || 'bili_monitor_logs');
  var conn;
  try {
    conn = await createMysqlConn(settings);
    ...
    for (var i=0;i<logs.length;i++) {
      var item = logs[i];
      await conn.query('INSERT INTO ' + table + ' (t, level, msg) VALUES (?, ?, ?)', [Number(item.t)||Date.now(), String(item.level||'info'), String(item.msg||'')]);
    }
    await conn.query('DELETE FROM ' + table + ' WHERE id NOT IN (SELECT id FROM (SELECT id FROM ' + table + ' ORDER BY id DESC LIMIT 1000) t)');
  } finally {
    if (conn) { try { await conn.end(); } catch (e) {} }
  }
}
async function clearLogsFromMysql(settings) {
  var table = mysqlEscapeId(settings.mysqlTable || 'bili_monitor_logs');
  var conn;
  try {
    conn = await createMysqlConn(settings);
...
async function addLog(env, level, msg) {
  var settings = await getSettings(env);
  if (settings.mysqlHost && settings.mysqlDatabase) {
    try {
...
async function handleApi(request, env) {
  ...
  if (path === '/api/logs/clear' && method === 'POST') {
    ...
    if (settings.mysqlHost && settings.mysqlDatabase) {
    try {
      await clearLogsFromMysql(settings);
      return json({ ok: true });
    } catch (e) {
