...
async function fetchUpName(mid, cookie, rsshubBase, env) {
  var cookie2 = await getFingerCookie(cookie);
  try {
    var resp = await fetch('https://api.bilibili.com/x/web-interface/card?mid=' + encodeURIComponent(String(mid)), { headers: biliHeaders(cookie2, 'https://space.bilibili.com/' + String(mid)) });
    var data = await resp.json();
    var name = data && data.data && data.data.card && data.data.card.name;
    if (name) return String(name).trim();
  } catch (e) {}
  var bases = getRsshubBases(rsshubBase);
  for (var i = 0; i < bases.length; i++) {
    try {
      var root = bases[i];
      var rssResp = await fetch(root + '/bilibili/user/video/' + encodeURIComponent(String(mid)), { headers: { 'User-Agent': UA, 'Accept': 'application/rss+xml, application/xml, text/xml, */*' } });
...
async function handleApi(request, env) {
  ...
  if (path === '/api/ups' && method === 'POST') {
    ...
    const up = {
      id: id,
      mid: mid,
      name: (await fetchUpName(mid, settings.cookie, settings.rsshubBase, env)) || mid,
      monitorVideo: true,
      monitorDynamic: true,
      notify: true,
      download: true
...
if (path === '/api/download-progress' && method === 'GET') {
  ...
  if (path === '/api/logs' && method === 'GET') {
    ...
    if (env.BILI_MONITOR_D1) {
      try {
        const d1Result = await getLogsFromD1(env, date);
        return json({ ok: true, logs: d1Result.logs, dates: d1Result.dates, source: 'd1' });
      } catch (e) {}
    }
  if (settings.logWebdavUrl) {
      try {
        const logs = await getLogsFromWebdav(settings);
        return json({ ok: true, logs: logs, dates: getLogDates(logs), source: 'webdav' });
      } catch (e) {
        return json({ ok: false, error: String(e && e.message || e), source: 'webdav' }, 500);
      }
    }
    var kvLogs = await kvGet(env, 'logs', []);
    return json({ ok: true, logs: kvLogs, dates: getLogDates(kvLogs), source: 'kv' });
  }
