...
const UI_HTML = [
"<!doctype html>","<html lang=\"zh-CN\">","<head>","<meta charset=\"utf-8\">","<meta name=\"viewport\" content=\"width=device-width, initial-scale=1\">","<title>B站UP主监控</title>","<style>","*{box-sizing:border-box}","body{margin:0;font-family:system-ui,Segoe UI,Microsoft YaHei,Arial,sans-serif;background:#f5f6f8;color:#202124}","header{background:#262a33;color:#fff;padding:18px 26px}","h1{margin:0;font-size:22px}",".wrap{max-width:1000px;margin:22px auto;padding:0 16px}",".card{background:#fff;border:1px solid #e2e5eb;border-radius:10px;padding:18px;margin-bottom:16px;box-shadow:0 1px 2px rgba(30,34,40,.04)}",".card h2{margin:0 0 12px;font-size:17px}","label{display:block;font-size:13px;color:#555;margin-bottom:5px}","input,textarea{width:100%;padding:9px 10px;border:1px solid #ccd0d8;border-radius:6px;font-size:14px;font-family:inherit}","textarea{resize:vertical;min-height:70px}",".row{display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:10px}",".check{display:flex;align-items:center;gap:6px;font-size:14px;margin:8px 0}",".check input{width:auto}","button{background:#2f6fed;color:#fff;border:0;border-radius:6px;padding:9px 14px;font-size:14px;cursor:pointer;margin-top:10px;margin-right:6px}","button:hover{background:#1f5ed8}","button.gray{background:#5f6672}","button.gray:hover{background:#4c525e}","button:disabled{opacity:.55;cursor:not-allowed}","table{width:100%;border-collapse:collapse;font-size:13px}","th,td{padding:8px;border-bottom:1px solid #e9ecf0;text-align:left;vertical-align:middle}","th{color:#555;font-weight:600;background:#fafbfc}",".log{max-height:380px;overflow:auto;background:#171a21;color:#d4d8e0;border-radius:8px;padding:12px;font-size:12px;font-family:Consolas,Menlo,monospace}",".log div{padding:3px 0;border-bottom:1px solid #272b35}",".log .error{color:#ff8080}.log .info{color:#8fb8ff}",".toast{position:fixed;right:18px;bottom:18px;background:#20242c;color:#fff;padding:10px 14px;border-radius:8px;opacity:0;transition:opacity .2s;pointer-events:none}",".toast.show{opacity:1}","</style>","</head>","<body>","<header><h1>📡 B站UP主监控</h1></header>","<div class=\"wrap\">","<div class=\"card\"><h2>全局设置</h2>","<label>B站 Cookie（建议粘贴含 SESSDATA 的完整Cookie）</label>","<textarea id=\"cookie\"></textarea>","<div class=\"row\"><div><label>企业微信 Webhook</label><input id=\"wecomWebhook\" placeholder=\"https://qyapi.weixin.qq.com/cgi-bin/webhook/send?key=...\"></div><div><label>下载接口前缀</label><input id=\"downloadApi\"></div></div>","<div class=\"row\"><div><label>WebDAV 地址</label><input id=\"webdavUrl\" placeholder=\"https://dav.example.com/dav/\"></div><div><label>检查间隔（分钟）</label><input id=\"intervalMinutes\" type=\"number\" min=\"1\"></div></div>","<div class=\"row\"><div><label>WebDAV 用户名</label><input id=\"webdavUser\"></div><div><label>WebDAV 密码</label><input id=\"webdavPass\" type=\"password\"></div></div>","<label class=\"check\"><input id=\"notifyOnFirstRun\" type=\"checkbox\"> 首次运行时也推送已存在的视频/动态</label>","<button id=\"saveBtn\">保存设置</button> <button id=\"testWecomBtn\" class=\"gray\">测试企微</button> <button id=\"testWebdavBtn\" class=\"gray\">测试WebDAV</button>","</div>","<div class=\"card\"><h2>UP主管理</h2>","<div class=\"row\"><div><label>UID 或空间链接</label><input id=\"newMid\" placeholder=\"例如 946974 或 https://space.bilibili.com/946974\"></div><div><label>名称（可选）</label><input id=\"newName\"></div></div>","<button id=\"addBtn\">添加UP主</button>","<table style=\"margin-top:14px\"><thead><tr><th>UP主</th><th>UID</th><th>视频</th><th>动态</th><th>通知</th><th>下载</th><th></th></tr></thead><tbody id=\"upList\"></tbody></table>","</div>","<div class=\"card\"><div style=\"display:flex;justify-content:space-between;align-items:center\"><h2>运行日志</h2><div><button id=\"runBtn\">立即检查</button> <button id=\"refreshLogsBtn\" class=\"gray\">刷新日志</button></div></div>","<div id=\"logs\" class=\"log\">加载中...</div>","</div>","</div>","<div id=\"toast\" class=\"toast\"></div>","<script>","var state={settings:null};","function $(id){return document.getElementById(id);}","function toast(msg){var el=$(\"toast\");el.textContent=msg;el.className=\"toast show\";setTimeout(function(){el.className=\"toast\";},2200);}","function api(path,opts){opts=opts||{};opts.headers=Object.assign({\"Content-Type\":\"application/json\"},opts.headers||{});return fetch(path,opts).then(function(r){return r.json();});}","function collectSettings(){return {cookie:$(\"cookie\").value,wecomWebhook:$(\"wecomWebhook\").value,downloadApi:$(\"downloadApi\").value,webdavUrl:$(\"webdavUrl\").value,webdavUser:$(\"webdavUser\").value,webdavPass:$(\"webdavPass\").value,intervalMinutes:Number($(\"intervalMinutes\").value)||5,notifyOnFirstRun:!!$(\"notifyOnFirstRun\").checked,ups:state.settings?state.settings.ups:[]};}","function fillSettings(s){state.settings=s;$(\"cookie\").value=s.cookie||\"\";$(\"wecomWebhook\").value=s.wecomWebhook||\"\";$(\"downloadApi\").value=s.downloadApi||\"https://bili.kedaya.gq/api/download?url=\";$(\"webdavUrl\").value=s.webdavUrl||\"\";$(\"webdavUser\").value=s.webdavUser||\"\";$(\"webdavPass\").value=s.webdavPass||\"\";$(\"intervalMinutes\").value=s.intervalMinutes||5;$(\"notifyOnFirstRun\").checked=!!s.notifyOnFirstRun;renderUps();}","function makeTd(text){var td=document.createElement(\"td\");td.textContent=text;return td;}","function makeCheck(checked,onChange){var td=document.createElement(\"td\");var input=document.createElement(\"input\");input.type=\"checkbox\";input.checked=!!checked;input.addEventListener(\"change\",function(e){onChange(e.target.checked);});td.appendChild(input);return td;}","function renderUps(){var tb=$(\"upList\");tb.innerHTML=\"\";(state.settings.ups||[]).forEach(function(u){var tr=document.createElement(\"tr\");tr.appendChild(makeTd(u.name||\"未命名\"));tr.appendChild(makeTd(u.mid));tr.appendChild(makeCheck(u.monitorVideo!==false,function(v){u.monitorVideo=v;}));tr.appendChild(makeCheck(u.monitorDynamic!==false,function(v){u.monitorDynamic=v;}));tr.appendChild(makeCheck(u.notify!==false,function(v){u.notify=v;}));tr.appendChild(makeCheck(u.download!==false,function(v){u.download=v;}));var del=document.createElement(\"button\");del.textContent=\"删除\";del.className=\"gray\";del.style.marginTop=\"0\";del.addEventListener(\"click\",function(){deleteUp(u.id);});var td=document.createElement(\"td\");td.appendChild(del);tr.appendChild(td);tb.appendChild(tr);});}","function loadSettings(){api(\"/api/settings\").then(function(j){if(j&&j.ok){fillSettings(j.settings||{});}else{toast((j&&j.error)||\"加载失败\");}}).catch(function(){toast(\"加载失败\");});}","function saveSettings(){var data=collectSettings();api(\"/api/settings\",{method:\"POST\",body:JSON.stringify(data)}).then(function(j){if(j.ok){toast(\"已保存\");state.settings=j.settings||data;renderUps();}else{toast(j.error||\"保存失败\");}}).catch(function(){toast(\"保存失败\");});}","function addUp(){var mid=$(\"newMid\").value.trim();var name=$(\"newName\").value.trim();if(!mid){toast(\"请输入UID或链接\");return;}api(\"/api/ups\",{method:\"POST\",body:JSON.stringify({mid:mid,name:name})}).then(function(j){if(j.ok){toast(\"已添加\");$(\"newMid\").value=\"\";$(\"newName\").value=\"\";loadSettings();}else{toast(j.error||\"添加失败\");}}).catch(function(){toast(\"添加失败\");});}","function deleteUp(id){if(!confirm(\"确认删除该UP主？\"))return;api(\"/api/ups/delete\",{method:\"POST\",body:JSON.stringify({id:id})}).then(function(j){if(j.ok){toast(\"已删除\");loadSettings();}else{toast(j.error||\"删除失败\");}}).catch(function(){toast(\"删除失败\");});}","function runNow(){api(\"/api/check\",{method:\"POST\"}).then(function(j){toast(j.ok?\"检查完成\":\"检查失败\");loadLogs();}).catch(function(){toast(\"检查失败\");});}","function testWecom(){var data=collectSettings();api(\"/api/test-wecom\",{method:\"POST\",body:JSON.stringify(data)}).then(function(j){toast(j.ok?\"企微测试成功\":(j.error||\"测试失败\"));}).catch(function(){toast(\"测试失败\");});}","function testWebdav(){var data=collectSettings();api(\"/api/test-webdav\",{method:\"POST\",body:JSON.stringify(data)}).then(function(j){toast(j.ok?\"WebDAV连接成功\":(j.error||\"测试失败\"));}).catch(function(){toast(\"测试失败\");});}","function loadLogs(){api(\"/api/logs\").then(function(j){var box=$(\"logs\");box.innerHTML=\"\";if(!j||!j.ok){box.textContent=\"加载失败\";return;}var logs=j.logs||[];if(!logs.length){box.textContent=\"暂无日志\";return;}logs.forEach(function(l){var div=document.createElement(\"div\");div.className=l.level||\"info\";var t=new Date(l.t).toLocaleString(\"zh-CN\",{hour12:false});div.textContent=\"[\"+t+\"] \"+l.msg;box.appendChild(div);});}).catch(function(){var box=$(\"logs\");box.textContent=\"加载失败\";});}","$(\"saveBtn\").addEventListener(\"click\",saveSettings);","$(\"addBtn\").addEventListener(\"click\",addUp);","$(\"runBtn\").addEventListener(\"click\",runNow);","$(\"refreshLogsBtn\").addEventListener(\"click\",loadLogs);","$(\"testWecomBtn\").addEventListener(\"click\",testWecom);","$(\"testWebdavBtn\").addEventListener(\"click\",testWebdav);","loadSettings();loadLogs();setInterval(function(){loadLogs();},15000);","</script>","</body>","</html>"
