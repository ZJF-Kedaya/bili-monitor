...
'<button onclick="saveSettings()">保存设置</button> <button class="gray" onclick="testWecom()">测试企微</button> <button class="gray" onclick="testWebdav()">测试WebDAV</button>',
'</div>',
...
'<button onclick="addUp()">添加UP主</button>',
...
'</div>',
'<div class="card"><div style="display:flex;justify-content:space-between;align-items:center"><h2>运行日志</h2><div><button onclick="runNow()">立即检查</button> <button class="gray" onclick="loadLogs()">刷新日志</button></div></div>',
...
'function toast(msg){var el=$("toast");el.textContent=msg;el.classList.add("show");setTimeout(function(){el.classList.remove("show")},2200);}',
...
'function renderUps(){var tb=$("upList");tb.innerHTML="";(state.settings.ups||[]).forEach(function(u){var tr=document.createElement("tr");tr.innerHTML="<td>"+escapeHtml(u.name||"未命名")+"</td><td>"+escapeHtml(u.mid)+"</td>"+"<td><input type=\"checkbox\" "+(u.monitorVideo!==false?"checked":"")+" onchange=\"setUpFlag(\'"+u.id+"\',\'monitorVideo\',this.checked)\"></td>"+"<td><input type=\"checkbox\" "+(u.monitorDynamic!==false?"checked":"")+" onchange=\"setUpFlag(\'"+u.id+"\',\'monitorDynamic\',this.checked)\"></td>"+"<td><input type=\"checkbox\" "+(u.notify!==false?"checked":"")+" onchange=\"setUpFlag(\'"+u.id+"\',\'notify\',this.checked)\"></td>"+"<td><input type=\"checkbox\" "+(u.download!==false?"checked":"")+" onchange=\"setUpFlag(\'"+u.id+"\',\'download\',this.checked)\"></td>"+"<td><button class=\"gray\" style=\"margin-top:0\" onclick=\"deleteUp(\'"+u.id+"\')\">删除</button></td>";tb.appendChild(tr);});}',
'function escapeHtml(s){return String(s||"").replace(/[&<>]/g,function(c){return{"&":"&amp;","<":"&lt;",">":"&gt;"}[c];});}',
'function loadSettings(){api("/api/settings").then(function(j){if(j.ok){fillSettings(j.settings);}else{toast(j.error||"加载失败");}});}',
'function saveSettings(){var data=collectSettings();api("/api/settings",{method:"POST",body:JSON.stringify(data)}).then(function(j){if(j.ok){toast("已保存");state.settings=j.settings;renderUps();}else{toast(j.error||"保存失败");}}).finally(function(){});}',
'function addUp(){var mid=$("newMid").value.trim();var name=$("newName").value.trim();if(!mid){toast("请输入UID或链接");return;}api("/api/ups",{method:"POST",body:JSON.stringify({mid:mid,name:name})}).then(function(j){if(j.ok){toast("已添加");$("newMid").value="";$("newName").value="";loadSettings();}else{toast(j.error||"添加失败");}});}',
'function deleteUp(id){if(!confirm("确认删除该UP主？"))return;api("/api/ups/delete",{method:"POST",body:JSON.stringify({id:id})}).then(function(j){if(j.ok){toast("已删除");loadSettings();}else{toast(j.error||"删除失败");}});}',
'function setUpFlag(id,field,val){var u=(state.settings.ups||[]).find(function(x){return x.id===id;});if(u){u[field]=val;}renderUps();toast("已更新，别忘保存设置");}',
'function runNow(){api("/api/check",{method:"POST"}).then(function(j){if(j.ok){toast("检查完成");loadLogs();}else{toast(j.error||"检查失败");}});}',
'function testWecom(){var data=collectSettings();api("/api/test-wecom",{method:"POST",body:JSON.stringify(data)}).then(function(j){toast(j.ok?"企微测试成功":(j.error||"测试失败"));});}',
'function testWebdav(){var data=collectSettings();api("/api/test-webdav",{method:"POST",body:JSON.stringify(data)}).then(function(j){toast(j.ok?"WebDAV连接成功":(j.error||"测试失败"));});}',
'function loadLogs(){api("/api/logs").then(function(j){var box=$("logs");if(!j.ok){box.innerHTML="加载失败";return;}if(!j.logs||!j.logs.length){box.innerHTML="暂无日志";return;}box.innerHTML=j.logs.map(function(l){var t=new Date(l.t).toLocaleString("zh-CN",{hour12:false});return "<div class=\""+escapeHtml(l.level||"info")+"\">["+t+"] "+escapeHtml(l.msg)+"</div>";}).join("");});}',
